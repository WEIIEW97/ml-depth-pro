...
{
#ifdef _WIN32
  // for c api compatible, avoid using c++17 features
  // auto [rgb_fp32_t, h, w] = preprocess_image(wideToString(img_path));

  cv::Mat rgb_fp32_t;
  int h, w;
  std::tie(rgb_fp32_t, h, w) = preprocess_image(wideToString(img_path));
#else
  // for c api compatible, avoid using c++17 features
  ...
  cv::Mat rgb_fp32_t;
  int h, w;
  std::tie(rgb_fp32_t, h, w) = preprocess_image(img_path);
#endif
